    ...
    def slow_nodismiss(event, interact=True, **kwargs):
        if not persistent.monika_kill:
            try:
                ...
                _window_hide(None)
                pause(2.0)
                renpy.jump("chmonika_end")
            if  config.skipping:
                persistent.tried_skip = True
                config.skipping = False
                config.allow_skipping = False
                renpy.jump("chmonika_noskip")
                return
...
label chmonika_noskip:
    ...
    if persistent.current_monikatopic != 0:
        ...
        call expression "chmonika_" + str(persistent.current_monikatopic)
    jump chmonika_loop
    return
...
label chmonika_main:
    $ persistent.autoload = "chmonika_main"
    $ config.allow_skipping = False
...
label chmonika_main2:
    if persistent.autoload == "chmonika_main2":
        $ config.allow_skipping = False
        $ m.display_args["callback"] = slow_nodismiss
        $ m.what_args["slow_abortable"] = config.developer
        if not config.developer:
        ...
        $ persistent.autoload = "chmonika_main2"
        $ renpy.save_persistent()
    menu:
        "Yes.":
            pass
...
label chmonika_postpoem:
    $ persistent.autoload = "chmonika_postpoem"
    $ renpy.save_persistent()
    $ m.display_args["callback"] = slow_nodismiss
    $ m.what_args["slow_abortable"] = config.developer
    $ config.skipping = False
    ...
    m "Let's see..."
    $ persistent.autoload = "chmonika_autoload"
    $ renpy.save_persistent()
    jump chmonika_loop


label chmonika_stream:
...
label chmonika_end:
    $ persistent.autoload = "chmonika_end"
...
label chmonika_endb:
    ...
    m "Goodbye."
label chmonika_end_2:
    $ persistent.autoload = "chmonika_end_2"
    $ m.display_args["callback"] = slow_nodismiss
    $ m.what_args["slow_abortable"] = config.developer
    $ style.say_dialogue = style.default_monika
    $ m_name = glitchtext(12)
...
label chmonika_autoload:
    ...
    if persistent.monika_reload <= 4:
        call expression "chmonika_reload_" + str(persistent.monika_reload)
    else:
        call chmonika_reload_4
